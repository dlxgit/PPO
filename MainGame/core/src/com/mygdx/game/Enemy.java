...
public class Enemy {
    ...
    {
        Vector2 startPos = sprite.getPosition();
    }
    ...
    Throwing CreateThrowing(Sprite object, Sprite target, String type,Sprite throwingSprite, float time)
    {
        ...
        Vector2 distance = ComputeDistanceBetweenSprites(object, target);
        ...
        throwing.stepPerLoop = { throwing.stepPerLoop.x * Constants.ENEMY_AXE_THROWING_STEP_PER_LOOP, throwing.stepPerLoop.y * Constants.ENEMY_AXE_THROWING_STEP_PER_LOOP };
    ...
    {
        Vector2 zombieCenter = GetSpriteCenter(enemy.sprite);

        for (Npc npc : npcList)
        {
            if (npc.state == LIVING)  //if loot.item.center contains heroSprite  -> add new item in inventory
            {
                Vector2 npcCenter = GetSpriteCenter(npc.sprite);
    ...
    {
        if (Math.abs(enemy.pos.x - hero.pos.x) > ZOMBIE_VISION_DISTANCE || Math.abs(enemy.pos.y - hero.pos.y) > ZOMBIE_VISION_DISTANCE)
        {
            enemy.follow = false;
        }
        if (enemy.follow == false)
    ...
    boolean IsEnemyNearHero(Hero hero, Enemy enemy)
    {
        ...
        Vector2 zombieCenter = GetSpriteCenter(enemy.sprite);
        Vector2 heroCenter = GetSpriteCenter(hero.sprite);
